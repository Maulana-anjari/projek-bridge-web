					...
					<div class="card-header {{ ($match->kategori != 'Latihan') ? 'bg-dark' : '' }}">
						<div class="row align-items-center">
							<div class="col-8 {{ ($match->kategori != 'Latihan') ? 'text-white' : '' }}">
								<a href="/team-match/{{$match->id}}" class="show-team {{ ($match->kategori != 'Latihan') ? 'text-white' : 'text-dark' }}">{{ ucfirst($match->kegiatan) }}</a>
							</div>
							<div class="col-4">
								<div class="row">
									<div class="col-auto">
										<a href="/team-match/{{$match->id}}/edit" class="btn btn-sm mb-1 btn-outline-primary">
											Edit
										</a>
									</div>
									<div class="col-auto">
										<form action="/team-match/{{$match->id}}" method="post">
											@csrf
											@method('DELETE')
